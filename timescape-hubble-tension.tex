...
value---a shape tension that recurs across the wider void and backreaction family,
none of which reaches the local $\Ho$, and that is not relieved by injecting the
measured line-of-sight void structure, though a free smooth expansion history does
...
standardisations and four BAO treatments (from the DR1 ladder; DR2 only raises it)
locates these softenings precisely. Including the $\sim\!13\%$ CMB $\fvo$
systematic \cite{Nazer2015} on the BAO$+$CMB side---a systematic that the
$r_d$-independent BAO-only fit does not carry---drops the split to
...
substituted. Across the CMB-systematic-free, non-anchor subgrid (the paper's own SN
reductions against the BAO-only fit) the split retains a $3.1\sigma$ floor. A parametric bootstrap sharpens the exclusion: of 4000 mocks drawn from a
...
$\gtrsim3.2\sigma$ by parametric bootstrap, with a $3.1\sigma$ floor across the
CMB-systematic-free, non-anchor sensitivity subgrid), softening to
$\sim1.8$--$2.3\sigma$ once the CMB $\fvo$ systematic is included on the BAO$+$CMB
side---whichever of the paper's own SN reductions is used---and to $\sim1.2\sigma$
if the external cosmology-independent anchor is also substituted
...
($0.74$--$0.85$) at $4.8$--$6.6\sigma$ for the primary reduction (profile
parameter-shift; $\gtrsim3.2\sigma$ by parametric bootstrap, with a $3.1\sigma$
floor across the CMB-systematic-free, non-anchor sensitivity subgrid), softening to
$\sim1.8$--$2.3\sigma$ once the CMB $\fvo$ systematic is included on the BAO$+$CMB
side and to $\sim1.2\sigma$ if the external cosmology-independent anchor is also
substituted. The dressed-$\Ho$
...
$p=0.07$, versus $\Delta\chi^2\approx67$ for the tracker; Sec.~\ref{sec:losvoids}),
...
the author and will be deposited publicly on publication. The DR2 BAO$+$CMB
...
\bibitem{FreeHistory2025} V.~Zhygulin, ``Free-history timescape: freeing the void-fraction history from the one-parameter tracker'', companion paper (in preparation), \url{https://github.com/szhygulin/free-history-timescape}.
\end{thebibliography}
