...
$\ln B>5$ purely from the covariance treatment. That cosmology-independent
covariance of Seifert et al.\ (2025)---which rebuilds rather than discards the
off-diagonal terms---has recently been released publicly, so the sign of the
supernova preference is now settleable under our harness; pending that
reproduction we localise the preference to the covariance treatment and conclude
that the supernova evidence on timescape is contingent rather than robust. Third, we calibrate timescape against DESI DR2
...
peculiar-velocity-independent) terms. That covariance has recently been released
publicly,\footnote{\url{https://github.com/antosft/SNe-PantheonPlus-Analysis}
(pipeline and covariance construction; repository updated 2026).} so their
$\ln B>5$ preference and $\fvo=0.737$ are now reproducible under our
harness---the immediate next step, which we flag rather than carry out here (our
own $\fvo$ rails to $\approx0.85$, and our raw-Tripp $\beta\approx2.66$ is below
...
when the off-diagonal terms are discarded, while the proponents'
cosmology-independent covariance has since been released publicly, so this
preference is now directly reproducible under our harness---the step that would
settle the sign, which we flag as immediate future work rather than carry out
here.
...
Pantheon$+$ covariance of Seifert et al., recently released publicly, run under
our harness to settle the sign of the supernova preference---the immediate next
step now that the data requirement is met.
...
($\dBIC=+3.9$ to $+5.1$; exact $\ln B=+1.3$ to $+1.8$), with a timescape
preference appearing only when the off-diagonal covariance is discarded (where the
stat-only reduction gives $\ln B=5.4$--$5.9$ for timescape), so the supernova
evidence is contingent on the covariance treatment; the proponents'
cosmology-independent covariance is now public, making that sign imminently
settleable. A joint calibration against DESI DR2 BAO and the Planck
